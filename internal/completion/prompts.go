...
func PermissionToInstallPrompt(shell string) bool {
	granted := gui.ConfirmPrompt(fmt.Sprintf("do you want me to install the %s completion scripts on your machine?", shell), "", true)
	return granted
}
...
func ConfirmInstallationPath(path string, shell string) bool {
	correct := gui.ConfirmPrompt(fmt.Sprintf("is %s the correct path for installing %s completions?", path, shell), "", true)
	return correct
}
